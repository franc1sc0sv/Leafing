...
<form method="POST" action="xDafgaq.php">
<div class="mb-3">
...
<a class="btn btn-primary mt-2" href="xDafgaq.php?mostrar_todo=ok">Muestra todo</a>
...
if(!empty($_POST))
{
      ...
      $filtro = "WHERE name_event LIKE LOWER('%".$aKeyword[0]."%')";
      $query ="SELECT * FROM `events` WHERE name_event LIKE LOWER('%".$aKeyword[0]."%')";
     ...
     if( mysqli_num_rows($result) > 0 AND $_POST['buscar'] != '') {
        ...
        While($row = $result->fetch_assoc()) {   
            $row_count++;   
            echo "<tr><td>".$row_count." </td><td>". resaltar_frase($row['name_event'] ,$_POST['buscar']) . "</td></tr>";
        }
        echo "</table>";
